...
function Home() {
    ...
    useEffect(() => {
        window.history.pushState({}, '', `/?q=${searchInput}&page=${page}`);
    }, []);

    let { data: list, setData, isPending, setIsPending, error } = useFetch(`${baseUrl}/anime?q=${searchInput}&page=${page}&limit=${limit}`)
    ...
    const updatePageNumber = (val) => {
        ...
        setData(null)
        setIsPending(true)
        window.history.pushState({}, '', `/?q=${searchInput}&page=${newPage}`);
    }
    ...
    const search = debounce((event) => {
        ...
        window.history.pushState({}, '', `/?q=${event.target.value}&page=${1}`);
    }, 500)

    const searchNoDebounce = (val) => {
        setPage(1)
        setLimit(25)
        setData(null)
        setIsPending(true)
        
        if(val) setSearchInput(val)
        else setSearchInput("")
    }
    ...
    return(
        <>
            <div className="w-full dark:bg-black duration-200 bg-white flex flex-col gap-5 py-10">
                <div className="w-full duration-200 dark:bg-black bg-white max-w-[1200px] mx-auto flex justify-center items-center">
                    <div className="border-neutral-300 duration-200 w-full max-w-[400px] rounded-full border-2 py-2 px-3 flex justify-between items-center">
                        <input onInput={search} placeholder="Type to search..." className="dark:text-white text-black peer w-full bg-transparent focus:outline-none" type="text" name="" id="" />
